...
const Header = () => {
  ...
  useEffect(() => {
    ...
    const CANVA_WIDTH = canvasRef.current.offsetWidth
    const CANVA_HEIGHT = canvasRef.current.offsetHeight
    ...
    class Enemy {
      constructor() {
        ...
        this.angleSpeed = Math.random() * 0.2 + 0.15
      ...
      update() {
        ...
        console.log(this.spriteWidth / 10)
        console.log(this.spriteHeight / 3)
    ...
    class Enemy2 {
      ...
      update() {
        this.x = 200 * Math.cos(this.angle * Math.PI / 200)
        this.x += this.speed
    ...
    class Enemy4 {
      ...
      update() {
        this.x = 70 * Math.sin(this.angle * Math.PI / 35) + (300 / 2 - this.width / 2)
        this.y = 60 * Math.cos(this.angle * Math.PI / 39) + (160 / 2 - this.height / 2)
        this.angle += this.angleSpeed
    ...
    class Enemy3 {
      ...
      update() {
        this.x = 60 * Math.sin(this.angle * Math.PI / 20) + (300 / 2 - this.width / 2)
        this.y = 50 * Math.sin(this.angle * Math.PI / 60) + (160 / 2 - this.height / 2)
        this.angle += this.angleSpeed
  ...
  return (
    <div className='header' style={{ position: 'relative', minHeight: '790px', minWidth: '1540px' }}>
      ...
      <div style={style} className='menu'>
        <canvas ref={canvasRef} style={{ position: 'absolute', width: '25%', height: '23%', bottom: '5%', right: '0%', }} id="canvas1"></canvas>
        <div style={{ position: 'absolute', width: '25%', height: '23%', bottom: '5%', left: '25%' }}>
          <div style={{ position: 'relative', overflow: 'hidden' }}>
            <img src={arbust} style={{ position: 'absolute', bottom: '1%', width: '70%', height: '70%', opacity: '90%', left: '34.5%' }}></img>
            <canvas ref={canvasRef1} style={{ width: '100%', height: '100%', bottom: '5%', left: '25%' }} id="canvas1"></canvas>
          </div>
        </div>
        <canvas ref={canvasRef2} style={{ position: 'absolute', width: '25%', height: '23%', bottom: '5%', left: '0%' }} id="canvas1"></canvas>
        <canvas ref={canvasRef3} style={{ position: 'absolute', width: '25%', height: '23%', bottom: '5%', right: '25%' }} id="canvas1"></canvas>
